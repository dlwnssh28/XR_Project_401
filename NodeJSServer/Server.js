...
wss.on('connection', function connection(ws) {
    ...
    ws.on('close', () => {
        ...
        if(index !== -1)
        {
            ...
            userList.slice(index, 1); //배열에서 해당 클라이언트 제거
        }

    });

    userList.push(ws.clientID);    // 새로 연결된 클라이언트를 유저리스트에 추가
...
function joinRoom(params, ws)
{
    ...
    {
        console.warn(`Room ${room} does not exist!`);   //룸이 없으면 존재하지 않는 다는 메세지
        return;
    }

    if(room[room].length >= maxClients)                 //5명 이상 못들어가게 막는다.
    {
        console.warn(`Room ${room} is full !`);   //룸이 없으면 존재하지 않는 다는 메세지
        return;
    }
